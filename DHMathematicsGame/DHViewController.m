...
#define DEFAULT_DURATION_FOR_REVEAL_IMAGE_ANIMATION 6.0f
...
- (void)initalizeAllView {
    ...
    self.instructionsNotificationsView = [[UIView alloc]
        initWithFrame:CGRectMake(self.view.center.x - 113,
                                 self.view.center.y - 275, 450, 450)];
    [self.instructionsNotificationsView
        addGestureRecognizer:self.panGestureRecognizer];
    self.generalAlertView = [[UIAlertView alloc] initWithTitle:@""
                                                       message:@""
                                                      delegate:nil
    ...
    self.instructionLabel = [[UILabel alloc]
        initWithFrame:CGRectMake(10, 10, self.instructionsNotificationsView
                                                 .frame.size.width -
                                             20,
                                 300)];
    ...
    self.dragToMoveLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(10, 5, 450, 30)];
    self.dragToMoveLabel.font =
        [DHColorDefaults getDefaultInstructionViewFontWithSize:24];
    self.dragToMoveLabel.text = @"Drag To Move";
    [self.instructionsNotificationsView addSubview:self.dragToMoveLabel];

    self.revealationImage = [[UIImageView alloc]
        initWithFrame:CGRectMake(
                          (self.instructionsNotificationsView.frame.size.width /
                           2) -
                              48,
                          225, 96, 96)];
    [self.instructionsNotificationsView addSubview:self.revealationImage];
    self.okButton = [[UIButton alloc]
        initWithFrame:CGRectMake(30, self.instructionLabel.frame.origin.y + 300,
                                 100, 60)];

    [self addBorderDesignToView:self.revealationImage
                       andColor:[DHColorDefaults getLightGreenColor]
                 andBorderWidth:2.0f];
    [self.okButton setBackgroundColor:[DHColorDefaults getLightOrangeColor]];


    [self.proceedButton addTarget:self
                           action:@selector(proceedButtonPressed:)
                 forControlEvents:UIControlEventTouchUpInside];

    [self showInstructionViewWithText:@"Select any option to get started"
                   andIsRevealingView:NO];
...
- (void)setupResetButton {
    ...
    self.resetButton = [[UIButton alloc]
        initWithFrame:CGRectMake(20, self.instructionLabel.frame.origin.y + 350,
                                 150, 60)];
    self.resetButton.alpha = 0.0;
    [self.resetButton setBackgroundColor:[DHColorDefaults getLightOrangeColor]];

    [self.resetButton setTitle:@"Reset" forState:UIControlStateNormal];
    [self.resetButton.titleLabel
        setFont:[DHColorDefaults getDefaultInstructionViewFontWithSize:24.0f]];
...
- (void)setupWrongAnswerButton {
    self.wrongAnswerButton = [[UIButton alloc]
        initWithFrame:CGRectMake(220,
                                 self.instructionLabel.frame.origin.y + 350,
                                 200, 60)];
    self.wrongAnswerButton.alpha = 0.0;
    [self.wrongAnswerButton
        setBackgroundColor:[DHColorDefaults getLightOrangeColor]];
    [self.wrongAnswerButton setTitle:@"Wrong Answer"
                            forState:UIControlStateNormal];
    [self.wrongAnswerButton.titleLabel
        setFont:[DHColorDefaults getDefaultInstructionViewFontWithSize:24.0f]];
    [self.wrongAnswerButton addTarget:self
                               action:@selector(userSaidWrongAnswer:)
                     forControlEvents:UIControlEventTouchUpInside];
...
- (void)handlePanFrom:(UIPanGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        // Current way
        self.initialX =
            self.instructionsNotificationsView.frame.origin.x + translation.x;
        self.initialY =
            self.instructionsNotificationsView.frame.origin.y + translation.y;

        // Alternate way to make movement immediately on drag
        //        self.instructionsNotificationsView.frame = CGRectMake
        //        (self.initialX+ translation.x , self.initialY+translation.y,
        //        self.instructionsNotificationsView.frame.size.width,
        //        self.instructionsNotificationsView.frame.size.height);


        DLog(@"Moving With X Value %f and Y Value %f", self.initialX,
             self.initialY);

    } else if (recognizer.state == UIGestureRecognizerStateEnded) {


        [UIView animateWithDuration:DEFAULT_DURATION_FOR_ANIMATION / 2
                              delay:0
                            options:UIViewAnimationOptionCurveEaseOut
                         animations:^{

                             // Current way
                             self.instructionsNotificationsView.frame =
                                 CGRectMake(self.initialX, self.initialY,
                                            self.instructionsNotificationsView
                                                .frame.size.width,
                                            self.instructionsNotificationsView
                                                .frame.size.height);
        ...
        DLog(@"Movement finished With X Value %f and Y Value %f", translation.x,
             translation.y);
    }
}
...
- (void)resetParameters {
    ...
    NSInteger numberOfTotalImagesOnCollectionView =
        NUMBER_OF_ITEMS_IN_COLLECTION_VIEW;
    ...
    self.okButton.frame = CGRectMake(
        10, self.instructionLabel.frame.origin.y + 330,
        self.instructionsNotificationsView.frame.size.width - 20, 60);
    ...
    while (numberOfTotalImagesOnCollectionView) {
        [self.imageSequenceStorage
            addObject:[NSString stringWithFormat:@"%ld",
                                                 (long)[DHUtilityMethodsProvider
                                                         getRandomNumber]]];
        numberOfTotalImagesOnCollectionView--;
    }

    // Initializing index paths
    self.previousSelectedRowInPrimaryTable =
        [NSIndexPath indexPathForRow:-1 inSection:0];
    self.selectedRowInPrimaryTable =
        [NSIndexPath indexPathForRow:-1 inSection:0];
    // Decide, which image to show up when revealed - Gets reset each time and
    // is selected randomely
    self.fixedImageNumber = [DHUtilityMethodsProvider getRandomNumber];

    [self configureInstructionViewWithImage:
              @"" andInstrcutionTitle:@"Please follow the Instructions View"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    // For each cell we will make sure cell falls in pre selected category then
    // just make background colorful. Otherwise just keep it white

    DHCollectionViewIconCell *iconCell = [self.mainCollectionView
    ...
    if ([self isNumberDivisibleBy9WithInputNumber:indexPath.row]) {
        ...
        [iconCell.image
            setImage:[UIImage imageNamed:[NSString
                                             stringWithFormat:
                                                 @"%ld.png",
                                                 (long)self.fixedImageNumber]]];
    } else {

        [iconCell.image
            setImage:[UIImage imageNamed:[NSString stringWithFormat:
                                                       @"%@.png",
                                                       self.imageSequenceStorage
                                                           [indexPath.row]]]];
    }
...
- (void)collectionView:(UICollectionView *)collectionView
    ...
    if (!self.didUserSelectOption) {
        ...
        [UIView transitionWithView:nil
                        ...
                        animations:^{ self.proceedButton.alpha = 0.8; }
                        completion:NULL];
    }
    ...
    if (self.previousSelectedRowInPrimaryTable.row != -1 &&
        self.previousSelectedRowInPrimaryTable.row !=
            self.selectedRowInPrimaryTable.row) {
        ...
        [self.mainCollectionView
            reloadItemsAtIndexPaths:@[ self.selectedRowInPrimaryTable ]];
    }
    //}
...
- (BOOL)isNumberDivisibleBy9WithInputNumber:(NSInteger)inputNumber {
    ...
    return [self isNumberDivisibleBy9WithInputNumber:((int)(inputNumber >> 3) -
                                                      (int)(inputNumber & 7))];
}

- (void)configureInstructionViewWithImage:(NSString *)imageName
                      andInstrcutionTitle:(NSString *)instructionText {
...
- (IBAction)resetButtonPressed:(id)sender {
    ...
    [UIView transitionWithView:nil
                      duration:DEFAULT_DURATION_FOR_ANIMATION
                       options:UIViewAnimationOptionTransitionCrossDissolve
                    animations:^{ [self hideInstructionView]; }
                    completion:NULL];
}
...
- (IBAction)proceedButtonPressed:(id)sender {
    ...
    if (self.didUserSelectOption) {
        ...
        self.instructionLabel.text =
            @"Now Reverse the number and then substract original number from "
            @"the value of reveresed number. After that, press reveal button "
            @"to reveal image adjacent to that number on screen";
        [self.okButton setTitle:@"Reveal" forState:UIControlStateNormal];

        [self.okButton addTarget:self
                          action:@selector(revealSelectedOption:)
                forControlEvents:UIControlEventTouchUpInside];

        [UIView transitionWithView:nil
                          duration:DEFAULT_DURATION_FOR_ANIMATION
                           options:UIViewAnimationOptionTransitionCrossDissolve
                        animations:^{ [self showInstructionView]; }
                        completion:NULL];

    } else {


        [self showGeneralAlertViewWithTitle:@"No Option selected"
                                 andMessage:@"Please select an option to "
                                            @"reveal predication"];
    }
}

- (void)showGeneralAlertViewWithTitle:(NSString *)alertTitle
...
- (void)showInstructionViewWithText:(NSString *)text
                 andIsRevealingView:(BOOL)isRevealingView {


    [self.instructionsNotificationsView
        setBackgroundColor:[DHColorDefaults getLightVioletColor]];


    self.instructionLabel.backgroundColor =
        self.instructionsNotificationsView.backgroundColor;
    self.instructionLabel.numberOfLines = 8;
    self.instructionLabel.textAlignment = NSTextAlignmentCenter;
    self.instructionLabel.font =
        [DHColorDefaults getDefaultInstructionViewFontWithSize:24.0f];
    ...
    if (isRevealingView) {
        ...
        self.instructionsNotificationsView.frame =
            CGRectMake(self.mainCollectionView.center.x - 200, 150, 450, 450);
        self.instructionLabel.frame = CGRectMake(
            10, 10, self.instructionsNotificationsView.frame.size.width - 20,
            300);
    ...
    [self addCircularCornerToView:self.instructionsNotificationsView
                       WithRadius:30.0f];

    // Border to View
    [self addBorderDesignToView:self.instructionsNotificationsView
                       andColor:[DHColorDefaults getLightVioletColor]
                 andBorderWidth:2.0f];

    // drop shadow
    [self addShadowToView:self.instructionsNotificationsView
                withColor:[DHColorDefaults getLightVioletColor]
               andOpacity:1.0
                andRadius:3.0f
                andOffset:CGSizeMake(3.0f, 3.0f)];

    [self.view addSubview:self.instructionsNotificationsView];

    [UIView
        transitionWithView:nil
                  duration:DEFAULT_DURATION_FOR_ANIMATION
                   options:UIViewAnimationOptionTransitionCrossDissolve
                animations:^{ self.instructionsNotificationsView.alpha = 0.9; }
                completion:NULL];
}

- (IBAction)userSaidWrongAnswer:(UIButton *)sender {
    [self showGeneralAlertViewWithTitle:@"Cheater"
                             andMessage:@"You are lying or are really bad at "
                                        @"basic mathematical operators. Image "
                                        @"revealed by algorithm is indeed the "
                                        @"image you evaluated. Please be "
                                        @"careful next time and again check "
                                        @"for correctness"];
    [self performSelector:@selector(resetButtonPressed:) withObject:sender];
}

- (void)addCircularCornerToView:(UIView *)inputView
                     WithRadius:(float)borderRadius {
    [inputView.layer setCornerRadius:borderRadius];
}

- (void)addBorderDesignToView:(UIView *)inputView
                     andColor:(UIColor *)inputColor
               andBorderWidth:(float)borderWidth {
    [self.instructionsNotificationsView.layer
        setBorderColor:inputColor.CGColor];
    [self.instructionsNotificationsView.layer setBorderWidth:borderWidth];
}

- (void)addShadowToView:(UIView *)inputView
              withColor:(UIColor *)shadowColor
             andOpacity:(float)shadowOpacity
              andRadius:(float)shadowRadius
              andOffset:(CGSize)shadowOffset {
    [self.instructionsNotificationsView.layer
        setShadowColor:shadowColor.CGColor];
...
- (IBAction)revealSelectedOption:(id)sender {
    ...
    [self showInstructionViewWithText:@"Image corresponding to your answer is"
                   andIsRevealingView:YES];
}
...
- (void)showCalculationAnimationWithDuration:(float)waitingAnimationDuration {
    ...
    self.loadingAnimationHolderView =
        [[UIView alloc] initWithFrame:CGRectMake(0, 0, 450, 450)];
    self.loadingAnimationHolderView.alpha = 0.0;
    [self.loadingAnimationHolderView setBackgroundColor:[UIColor whiteColor]];


    UIImageView *greenLoadingCircle = [[UIImageView alloc]
        initWithFrame:CGRectMake(positionToGo, initialYPositionOfAnimationBalls,
                                 48, 48)];
    [greenLoadingCircle setImage:[UIImage imageNamed:@"circle1.jpeg"]];
    [self.loadingAnimationHolderView addSubview:greenLoadingCircle];
    // Green

    UIImageView *redLoadingCircle = [[UIImageView alloc]
        initWithFrame:CGRectMake(positionToGo - triangleSideLength,
                                 initialYPositionOfAnimationBalls +
                                     triangleHeight,
                                 48, 48)];
    ...
    UILabel *loadingLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(140, 200, 170, 60)];
    ...
    UILabel *secondLoadingLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(150, 200, 150, 60)];
    ...
    UILabel *randomNumberLabel =
        [[UILabel alloc] initWithFrame:CGRectMake(200, 270, 40, 40)];
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.15
                                   ...
                                   userInfo:@{
                                       @"numberLabel" : randomNumberLabel
                                   }
                                    repeats:YES];

    UIImageView *blueLoadingCircle = [[UIImageView alloc]
        initWithFrame:CGRectMake(positionToGo + triangleSideLength,
                                 initialYPositionOfAnimationBalls +
                                     triangleHeight,
                                 48, 48)];
    ...
    [UIView
        animateKeyframesWithDuration:2.0
                               delay:0.0
    ...
    [UIView
        animateKeyframesWithDuration:3.0
                               delay:0.0
                             options:UIViewKeyframeAnimationOptionAutoreverse |
                                     UIViewKeyframeAnimationOptionRepeat
                          animations:^{
                              ...
                              [UIView
                                  addKeyframeWithRelativeStartTime:4.0
                                                  relativeDuration:0.5
                                                        animations:^{
    ...
    [self addCircularCornerToView:self.loadingAnimationHolderView
                       WithRadius:30.0f];
    [self.instructionsNotificationsView
        addSubview:self.loadingAnimationHolderView];
    [self.instructionsNotificationsView
        bringSubviewToFront:self.loadingAnimationHolderView];
}
...
- (void)showNumbers:(NSTimer *)timer {
    ...
    randomNumberLabel.text =
        [NSString stringWithFormat:@"%d", arc4random() % 100];
}

- (void)hideInstructionView {


    self.instructionsNotificationsView.transform =
    ...
    [UIView animateWithDuration:DEFAULT_DURATION_FOR_ANIMATION
                     ...
                     animations:^() {
                         ...
                         // self.instructionsNotificationsView.center =
                         // self.view.center;
                         self.instructionLabel.transform =
                             CGAffineTransformMakeScale(0.0, 0.0);
                         // self.instructionLabel.center = self.view.center;
                         self.okButton.transform =
                             CGAffineTransformMakeScale(0.0, 0.0);
